---
title: "SOK-1004, høst 2022, Case 3"
author: "[276]"
format: pdf
echo: true
output: true
...
Jeg fortsår ikke helt oppgaven, hvilket tall vil du ha en forklaring av?
...
# Koden ble hentet fra: https://www.listendata.com/2015/06/r-keep-drop-columns-from-data-frame.html
...
df <- rename(df, kpi = "statistikkvariabel", produkt = "konsumgruppe", verdi = "value")

df2 <- df[df$dato > "2010-12-01" &
              df$dato < "2022-10-01",]

df2

# Koden er hentet fra: https://statisticsglobe.com/subset-data-frame-between-two-dates-in-r
...
df_vekter <- filter(df2, kpi == "Konsumprisindeks (vekter)")

df_vekter$delt1000 <- df_vekter$verdi/1000

df_delt1000 <- with(na.omit(df_vekter), rowsum(delt1000, dato))

df_delt1000

```

Det er et lite avvik, spesielt i 2011, men avviket ser ut til å minske for hvert år som går. I 2022 summeres det til 0.9809, så det er kommet veldig nært 1.
...
df_kpi <- filter(df2, kpi == "Konsumprisindeks (2015=100)")
...
df_kpi %>% 
  ...
  filter(produkt == "Elektrisitet inkludert nettleie") %>% 

  ggplot() +
 
  geom_point(aes(x=dato, y=verdi, color = produkt)) + 
  
  labs (title = "Vekstbidraget for elektrisitet, inkludert nettleie", y = "Verdien med hensyn til KPI", x = "År")+
  
  theme_bw()

```

Figuren viser at verdien av elektrisitet var fluktuerende men stigende til 2019, da verdien sank. Men fra 2020 med covid-19 situasjonen har verdien av elektrisitet økt kraftig, og er nå nesten 3 ganger så høy som før covid.
...
Det virker sannsynlig at høyere kraftpriser kan bidra til konsumprisvekst. Uten elektrisitet ville ikke samfunnet vårt fungert slik det er nå, så høyere priser kraftpriser burde føre til en økning i pris på de aller fleste varer. Uten kraft kan man ikke produserer, importere eller eksportere varer, fordi det er avhenging av denne kraften for å forenkle de forskjellige prossesene som gjør dette. Når kraften da blir dyrere, blir også disse prossesene dyrere å utføre. Dette fører da til en økning i pris for det ferdige produktet.
